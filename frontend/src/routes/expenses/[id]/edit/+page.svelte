<script lang="ts">
	...
	const returnTo = $derived(urlParams.get('returnTo') || '/expenses');
		...
		try {
			...
			if (response.ok) {
				...
				if (originalExpense) {
					...
					formData.type = originalExpense.type;
					formData.category = originalExpense.category;
					formData.amount = originalExpense.amount;
					formData.date = new Date(originalExpense.date).toISOString().split('T')[0]!;
					formData.mileage = originalExpense.mileage ?? undefined;
					formData.gallons = originalExpense.gallons ?? undefined;
...
<svelte:head>
	<title>Edit Expense - VROOM Car Tracker</title>
...
{:else if originalExpense}
	<div class="max-w-2xl mx-auto space-y-6">
		<!-- Header -->
		<div class="flex items-center gap-4">
			<button onclick={() => goto(returnTo)} class="btn btn-secondary p-2">
...
{:else}
	<div class="text-center py-12">
		...
		<button onclick={() => goto(returnTo)} class="btn btn-primary"> Back to Expenses </button>
	</div>
{/if}
