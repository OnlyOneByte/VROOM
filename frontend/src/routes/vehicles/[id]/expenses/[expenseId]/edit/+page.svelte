	...
	async function loadLastFuelExpense() {
		try {
			const response = await fetch(
				`/api/expenses/vehicles/${vehicleId}/expenses?type=fuel&limit=2`,
				{
					credentials: 'include'
				}
			);
...
{:else if vehicle && originalExpense}
	<div class="max-w-2xl mx-auto space-y-6">
		...
		<div class="flex items-center gap-4">
			<button onclick={() => goto(`/vehicles/${vehicleId}/expenses`)} class="btn btn-secondary p-2">
...
{:else}
	<div class="text-center py-12">
		...
		<button onclick={() => goto(`/vehicles/${vehicleId}/expenses`)} class="btn btn-primary">
			Back to Expenses
		</button>
	</div>
{/if}
