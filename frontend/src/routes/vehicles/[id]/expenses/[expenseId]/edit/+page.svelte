<script lang="ts">
	...
	import type { Vehicle, Expense, ExpenseType, ExpenseCategory, ExpenseFormData } from '$lib/types.js';
	...
	async function loadExpense() {
		try {
			...
			if (response.ok) {
				originalExpense = await response.json().then(result => result.data);
				...
				if (originalExpense) {
					...
					formData = {
						...
						date: new Date(originalExpense.date).toISOString().split('T')[0],
						mileage: originalExpense.mileage,
						gallons: originalExpense.gallons,
				...
				goto(`/vehicles/${vehicleId}/expenses`);
			}
		} catch (error) {
			appStore.addNotification({
				type: 'error',
	...
	async function loadLastFuelExpense() {
		try {
			const response = await fetch(`/api/expenses/vehicles/${vehicleId}/expenses?type=fuel&limit=2`, {
				credentials: 'include'
			});
			...
			if (response.ok) {
				...
				if (result.data && result.data.length > 0) {
					// Find the most recent fuel expense that's not the current one being edited
			...
			case 'mileage':
				if (formData.type === 'fuel') {
					...
					if (lastFuelExpense?.mileage && value <= lastFuelExpense.mileage) {
						return 'Mileage must be greater than previous fuel entry';
					}
...
<svelte:head>
	<title>Edit Expense - {getVehicleDisplayName()} - VROOM Car Tracker</title>
	<meta name="description" content="Edit vehicle expense" />
...
{:else if vehicle && originalExpense}
	<div class="max-w-2xl mx-auto space-y-6">
		<!-- Header -->
		<div class="flex items-center gap-4">
			<button 
				onclick={() => goto(`/vehicles/${vehicleId}/expenses`)}
...
{:else}
	<div class="text-center py-12">
		...
		<h3 class="text-lg font-medium text-gray-900 mb-2">Expense not found</h3>
		<p class="text-gray-600 mb-4">The expense you're looking for doesn't exist or you don't have access to it.</p>
		<button 
			onclick={() => goto(`/vehicles/${vehicleId}/expenses`)}
