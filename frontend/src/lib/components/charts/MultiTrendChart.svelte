...
<div class="w-full">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
		...
		<div class="flex flex-wrap gap-2">
			...
			<button
				class="px-3 py-1 text-sm rounded-md transition-colors {selectedMetrics.includes(
					'costPerMile'
				)
				...
				{#if selectedMetrics.includes('cost') && costData.length > 0}
					<div class="bg-blue-50 p-4 rounded-lg">
						...
						<div class="text-xs text-blue-600 mt-1">
							Avg: ${(
								costData.reduce((sum, d) => sum + (d.amount || 0), 0) / costData.length
							).toFixed(2)}/month
						</div>
					</div>
				{/if}
				...
				{#if selectedMetrics.includes('miles') && milesData.length > 0}
					<div class="bg-red-50 p-4 rounded-lg">
						...
						<div class="text-xs text-red-600 mt-1">
							Avg: {Math.round(
								milesData.reduce((sum, d) => sum + (d.miles || 0), 0) / milesData.length
				...
				{#if selectedMetrics.includes('costPerMile') && costPerMileData.length > 0}
					<div class="bg-green-50 p-4 rounded-lg">
						...
						<div class="text-2xl font-bold text-green-900">
							${(
								costPerMileData.reduce((sum, d) => sum + (d.costPerMile || 0), 0) /
								costPerMileData.length
							).toFixed(3)}
						</div>
						<div class="text-xs text-green-600 mt-1">
							Range: ${Math.min(...costPerMileData.map(d => d.costPerMile || 0)).toFixed(3)} - ${Math.max(
								...costPerMileData.map(d => d.costPerMile || 0)
							).toFixed(3)}
						</div>
					</div>
				{/if}
