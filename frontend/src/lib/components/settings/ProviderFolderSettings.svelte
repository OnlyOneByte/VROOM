<script lang="ts">
	...
	const CATEGORIES: PhotoCategory[] = ['vehicle_photos', 'expense_receipts', 'insurance_docs', 'odometer_readings'];
	...
	let rootPath = $state((provider.config?.['rootPath'] as string) ?? '/VROOM');
	let advancedOpen = $state(false);

	// Reset rootPath when provider changes
	$effect(() => {
		rootPath = (provider.config?.['rootPath'] as string) ?? '/VROOM';
	});
	...
	function handleRootPathBlur() {
		...
		const originalPath = (provider.config?.['rootPath'] as string) ?? '/VROOM';
		...
		saveTimeout = setTimeout(async () => {
			try {
				await providerApi.updateProvider(provider.id, {
					config: { ...provider.config, rootPath }
				});
			} catch {
		...
		<Collapsible.Content class="pt-3">
			<div class="space-y-3 rounded-lg border border-border p-3">
				...
				{#if defaultCategories.size > 0}
					<p class="text-xs text-muted-foreground">
						Categories marked as default cannot be disabled. Change the default provider in settings first.
					</p>
				{/if}
