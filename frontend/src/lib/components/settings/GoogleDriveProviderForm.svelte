<script lang="ts">
	...
	import { Cloud, CircleCheck } from '@lucide/svelte';
	...
	function handleConnectGoogle() {
		if (browser) {
			// Direct API redirect — not a SvelteKit route, so resolve() is not applicable.
			// Uses getApiBaseUrl() so it works behind reverse proxies with a base path.
			window.location.href = `${getApiBaseUrl()}/api/v1/auth/reauth/google`;
		}
	}
</script>
...
<div class="space-y-4">
	{#if connectedEmail}
		<div class="flex items-center justify-between gap-3 rounded-lg border border-border p-3">
			...
			<Button variant="ghost" size="sm" class="shrink-0" onclick={handleConnectGoogle}>
				Change
			</Button>
		</div>
	{:else}
