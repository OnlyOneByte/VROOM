			...
			<div class="flex items-center gap-3 min-w-0">
				...
				<div class="min-w-0">
					...
					<p class="text-xs text-muted-foreground">
						{getProviderTypeLabel(provider.providerType)} · Last sync: {formatLastSync(
							provider.lastSyncAt
						)}
					</p>
				</div>
			...
			<div class="space-y-3 mb-4">
				...
				{#if provider.providerType === 'google-drive'}
					<div class="text-sm">
						...
						<span>{(provider.config?.['accountEmail'] as string) ?? 'Connected'}</span>
					</div>
				{:else if provider.providerType === 's3'}
			...
			<div class="space-y-3">
				<div class="flex items-center justify-between">
					<p class="text-sm font-medium text-muted-foreground">
						Storage · <span class="font-mono text-xs">{(provider.config?.['rootPath'] as string) ?? '/VROOM'}</span>
					</p>
				...
				{:else}
					{#each Object.entries(categorySettings) as [category, setting] (category)}
						...
						{@const pct = status && status.total > 0 ? Math.round((status.synced / status.total) * 100) : 0}
