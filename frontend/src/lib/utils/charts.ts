...
export function createLineChart(
  ...
  g.selectAll('.dot')
    .on('mouseover', function(event, d: any) {
      ...
      tooltip.html(`
        <div>Date: ${d3.timeFormat('%b %Y')(d.date)}</div>
        <div>Amount: $${(d.amount || 0).toFixed(2)}</div>
      `)
  ...
  g.selectAll('.bar')
    .on('mouseover', function(event, d: any) {
      ...
      tooltip.html(`
        ...
        <div>Amount: $${d.amount.toFixed(2)}</div>
        <div>Count: ${d.count} expenses</div>
...
export function createPieChart(
  ...
  const labelArc = d3.arc<d3.PieArcDatum<CategoryData>>()
  ...
  slices.selectAll('path')
    .on('mouseover', function(event, d: any) {
      ...
      tooltip.html(`
        ...
        <div>Amount: $${d.data.amount.toFixed(2)}</div>
        <div>Count: ${d.data.count} expenses</div>
...
export function createMultiLineChart(
  ...
  const parseDate = d3.timeParse('%Y-%m');
  const allData: Array<{ key: string; values: Array<{ date: Date; value: number; period: string }> }> = [];
