...
- Standing goal (TODO.md → Misc): raise test coverage to **90%** both sides. Latest MEASURED reading
  (re-measured C460, not an estimate): **backend 87.09% line / 86.60% func · frontend 85.89% line / 87.15%
  func / 78.35% branch** (both suites > 85% line; BE UP vs the C447 reading 86.96/86.55 — the C453 0%-APR + C458 split-cents + C459 notification-feed fix+guards; FE flat vs C447 as the C450/C455/C456 cycles were a 1-test guard / a 3-line component re-point / BE-side). BE↔FE gap now **~1.2pts — the tightest era**. The 90% goal stays structurally gated: BE tail is DI/singleton-bound + OAuth-network; FE gap is the eyes-on components/routes deficit (Playwright-blocked).
  ...
  **The FE SERVICE + pure-util + STORE layers are now fully covered** (api-client + expense-api + reminder-api +
  ...
  Suite size today: **~1558 backend tests / ~715 frontend** (a floor — grows most cycles; the C459–C466 arc added
  the C459 notification-feed-order + C463 mileage-due-equality-boundary + C465 split-source-link (×4) + C466
  split-sibling-fillup-cost (×3) guards + the C462 formatRelativeTime singular grammar, on top of the C440–C458 arc).
  Don't regress coverage; name why if a cycle drops it.
...
- Loop-found HIGHs now CLOSED: **#27** (TCO financed-principal double-count) fixed C154 (Angelo-approved
  ...
  reminder notification feed ordered by desc(dueDate) but mileage notifications carry dueDate=NULL (milestone in dueOdometer) → NULLs sort LAST → a just-due mileage notification sank below year-old time ones AND was truncated past limit(100); order by desc(createdAt) [#142, C459, feature-disabling for the maintenance mileage axis],
  the background photo sync-worker retried a TERMINAL auth error (revoked token → SyncError AUTH_INVALID, which the adapters emit "so the user re-connects") as a transient flake — processSingleRef's catch ignored error.code → 3 backoff retries then stranded failed with no reconnect signal; branch on isSyncError && code∈{AUTH_INVALID,PERMISSION_DENIED} → park at the retry cap + "Reconnect required" message [#144, C461, the #105/#43/#44 fail-open family at the consumer leg],
  the manual SPLIT expense route (POST /expenses/split) accepted sourceType:z.string().optional() (any string) + ran NO assertFinancingSourceValid while the regular POST/PUT restrict to the 'financing' literal — a hand-crafted split could forge an unvalidated financing link (understates the loan balance) or an insurance_term link (cascade-deletes the manual split when that policy is removed); tighten the schema to z.literal('financing') + assertFinancingSourceValid per split vehicle [#145, C465, the #62/#109/#125 within-tenant source-link class on the THIRD path],
  buildFillupCostByVehicle counted EVERY category='fuel' row with no isFillup guard → a split fuel expense's volume=null per-vehicle cost-allocation sibling was counted as a standalone fillup, diluting the "Avg Fillup Cost by Vehicle" chart; add `if (!isFillup(row)) continue;` [#146, C466, the #56/#108/#113 split-sibling overcount class on the builder the C391 sweep missed])
  all landed C155–C466. Also a clean grammar fix (#143/C462 formatRelativeTime "1 weeks/months/years ago" → singular at each bucket edge) + an arch dead-code delete (C464, the dead Lucia setTestLucia/testLucia override seam). Recurring lesson the loop keeps re-finding (C181/C182/C185/C229):
...
- Pending an Angelo decision (filed, NOT auto-fixed — each changes a displayed $/HTTP behavior or is a
  ...
  **#140 LeaseMetricsCard annual-vs-total** — ✅ **CLOSED (already fixed; verified C68 2026-06 eyes-on + confirmed firsthand C240).** All 3 LeaseMetricsCard display sites route through `leaseTotalMileageAllowance` (LeaseMetricsCard.svelte:42; the only `mileageLimit` use is the `{#if}` presence-guard, NOT a bare annual comparison). It was removed from the live `loop/BACKLOG.md` queue C1 (2026-06-17) as post-reset doc-drift; this stale "still OPEN" note (carried from the C447→C466 era) was itself stale — do NOT re-pick it. (NOTE: the SEPARATE #148 — LeaseMetricsCard burn bar reads 0-used when initialMileage is null — IS open + Angelo-escalated, a different axis; see loop/BACKLOG.md.)
  (CLOSED since the C447 refresh [now historical]: #142 notification-feed-order [C459], #143 formatRelativeTime singular grammar [C462], #144 sync-worker terminal-auth retry [C461], #145 split-route forgeable source-link [C465], #146 split-sibling fillup-cost overcount [C466]. The #62/#109/#125 within-tenant source-link integrity class is now closed across ALL three write paths (regular POST, regular PUT, split); the #56/#108/#113 split-sibling overcount class swept across all fuel builders. The live OPEN queue is `loop/BACKLOG.md`, NOT this note — this CLAUDE.md bug list is a point-in-time snapshot, kept for grounding but superseded by the loop docs.)
