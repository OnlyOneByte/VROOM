...
- Standing goal (TODO.md → Misc): raise test coverage to **90%** both sides. Latest MEASURED reading
  ...
  **The FE SERVICE + pure-util layers are now essentially fully covered** (api-client + expense-api + reminder-api +
  ...
  Suite size today: **~1440 backend tests / ~641 frontend** (a floor — grows most cycles; C322–C334 added
  the #94-volume-pooling pin, the #99 month-overflow-clamp guards, getSyncStatusInfo, the year-scoped TCO
  pins, the per-field import-conversion guards). Don't regress coverage; name why if a cycle drops it.
...
- Loop-found HIGHs now CLOSED: **#27** (TCO financed-principal double-count) fixed C154 (Angelo-approved
  ...
  all landed C155–C330. Recurring lesson the loop keeps re-finding (C181/C182/C185/C229):
