...
- Standing goal (TODO.md → Misc): raise test coverage to **90%** both sides. Latest MEASURED reading
  (re-measured C423, not an estimate): **backend 86.93% line / 86.57% func · frontend 84.51% line / 84.54%
  func / 77.06% branch** (both suites > 84% line; flat-to-UP vs the C412 reading 86.92/86.56 · 84.46/84.4/76.92 — the C413–C428 fix+guard arc [the #119/#122/#126 PHEV-MPG gas/charge partition (gasEfficiencyPoint across analytics-charts AND repository converted/trend paths), #123 provider-PUT, #124 import-decimal, #125 financing-source-PUT, #127 restore-uniqueness fixes + the gasEfficiencyPoint/resetSplitAllocations/sortExpensesByDate/getPendingExpenses extracts + the C414/C419/C425 EV-partition/MPG-band/financing-POST guards] held/nudged the line as product code grew). BE↔FE gap ~2.4pts (stable, the tightest era). The 90% goal stays structurally gated: BE tail is DI/singleton-bound + OAuth-network; FE gap is the eyes-on components/routes deficit (Playwright-blocked).
  ...
  **The FE SERVICE + pure-util + STORE layers are now fully covered** (api-client + expense-api + reminder-api +
  ...
  Suite size today: **~1526 backend tests / ~697 frontend** (a floor — grows most cycles; the C418–C428 arc added
  the #124 import-decimal, #125 financing-source-PUT, #126 converted-path PHEV-MPG, #127 restore-uniqueness fixes
  + the sortExpensesByDate (C421) / getPendingExpenses (C426) extracts + the C419 MPG-band + C425 financing-POST
  guards, on top of the C406–C417 arc [#C404/#118/#119/#122/#123 + C407/C414/C415 guards]). Don't regress coverage; name why if a cycle drops it.
...
- Loop-found HIGHs now CLOSED: **#27** (TCO financed-principal double-count) fixed C154 (Angelo-approved
  ...
  all landed C155–C428. Recurring lesson the loop keeps re-finding (C181/C182/C185/C229):
