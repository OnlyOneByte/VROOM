...
- Standing goal (TODO.md → Misc): raise test coverage to **90%** both sides. Latest MEASURED reading
  (re-measured C435, not an estimate): **backend 86.94% line / 86.60% func · frontend 85.26% line / 85.53%
  func / 77.40% branch** (both suites > 85% line; UP vs the C423 reading 86.93/86.57 · 84.51/84.54/77.06 — the C430–C434 fix+guard arc [the C430 calculateVehicleStats outlier-band + C434 #76-third-leg guards on BE; the C426/C432 sync-manager dead-code/dup removal RAISED the FE covered ratio +0.75 line] held/lifted the line as product code grew). BE↔FE gap now **~1.7pts — the tightest ever**. The 90% goal stays structurally gated: BE tail is DI/singleton-bound + OAuth-network; FE gap is the eyes-on components/routes deficit (Playwright-blocked).
  ...
  **The FE SERVICE + pure-util + STORE layers are now fully covered** (api-client + expense-api + reminder-api +
  ...
  Suite size today: **~1533 backend tests / ~707 frontend** (a floor — grows most cycles; the C430–C439 arc added
  the C430 outlier-band guard, #128 getMaterializedExpenses shape fix, #130 #76-third-leg fix+guard, the C436 +9
  chart-formatters guard cluster, #131 ReminderForm date fix + source-scan guard, the C439 year-scoped costPerMonth
  divisor guard, on top of the C418–C428 arc). Don't regress coverage; name why if a cycle drops it.
...
- Loop-found HIGHs now CLOSED: **#27** (TCO financed-principal double-count) fixed C154 (Angelo-approved
  ...
  all landed C155–C437. Recurring lesson the loop keeps re-finding (C181/C182/C185/C229):
...
- Pending an Angelo decision (filed, NOT auto-fixed — each changes a displayed $/HTTP behavior or is a
  ...
  **#129 OAuth-login email-sync** (MED, data-integrity, found+filed C433 on an auth deep-review — loop-fixable but a product nuance): the OAuth login callback (updateExistingUserProfile, auth/routes.ts:176) OVERWRITES users.email with the provider's reported email on EVERY login; the UNIQUE-collision branch is correct but a within-account email drift (the user changed their GitHub/Google primary email → next login silently changes their VROOM login email) is silent. Bounded (no cross-user takeover — collision handled). The fix is a 1-line product call: don't sync on login / sync only if unset / surface a "your email changed" notice. Queued in the bug list awaiting the steer.
  (CLOSED since the last refresh: #128 getMaterializedExpenses shape [C431], #130 #76-third-leg PUT mileage [C434], #131 ReminderForm UTC date [C437] — all landed, see the fix list above.)
