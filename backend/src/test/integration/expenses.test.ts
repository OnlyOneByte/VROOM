...
import { users, sessions, vehicles, expenses } from '../../db/schema';
...
describe('Expense CRUD Operations', () => {
    test('should create a new expense', async () => {
      ...
      expect(data.data.amount).toBe(45.50);
 ...
 test('should list vehicle expenses', async () => {
    ...
    test('should filter expenses by type', async () => {
      ...
      const req = new Request(`http://localhost:3001/api/expenses/vehicles/${testVehicleId}/expenses?type=fuel`, {
        headers: {
          Cookie: sessionCookie,
        },
      });
   ...
   test('should filter expenses by date range', async () => {
      ...
      const req = new Request(`http://localhost:3001/api/expenses/vehicles/${testVehicleId}/expenses?startDate=2024-01-01T00:00:00.000Z&endDate=2024-01-31T23:59:59.999Z`, {
        headers: {
          Cookie: sessionCookie,
        },
      });
...
('should update expense', async () => {
  ...
  describe('Fuel Efficiency Tracking', () => {
    ...
    test('should detect efficiency alerts for significant MPG drops', async () => {
      ...
      expect(data.data.alerts[0].currentMPG).toBeLessThan(data.data.alerts[0].averageMPG);
    });
  });});
