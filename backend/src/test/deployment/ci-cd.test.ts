...
describe('GitHub Actions Workflow Tests', () => {
  ...
  test('should have backend test job configured', () => {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: YAML step structure is dynamic
  ...
  test('should have proper Docker build configuration', () => {
    const buildBackend = workflowConfig.jobs['build-backend'];
    // biome-ignore lint/suspicious/noExplicitAny: YAML step structure is dynamic
    const buildStep = buildBackend.steps.find((s: any) => s.name === 'Build and push Docker image');
  ...
  test('should have proper image tagging strategy', () => {
    const buildBackend = workflowConfig.jobs['build-backend'];
    // biome-ignore lint/suspicious/noExplicitAny: YAML step structure is dynamic
    const metaStep = buildBackend.steps.find((s: any) => s.name === 'Extract metadata');
...
describe('Workflow Validation Tests', () => {
  ...
  test('should have all required GitHub Actions versions specified', () => {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: YAML step structure is dynamic
    const checkoutStep = backendTest.steps.find((s: any) => s.uses?.includes('actions/checkout'));
    expect(checkoutStep.uses).toMatch(/@v\d+/);

    // biome-ignore lint/suspicious/noExplicitAny: YAML step structure is dynamic
    const setupNode = frontendTest.steps.find((s: any) => s.uses?.includes('actions/setup-node'));
...
describe('CI/CD Best Practices Tests', () => {
  ...
  test('should use GitHub token for registry authentication', () => {
    ...
    // biome-ignore lint/suspicious/noTemplateCurlyInString: Testing GitHub Actions syntax
