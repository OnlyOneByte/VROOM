...
import { users, vehicles, expenses, insurancePolicies, vehicleLoans, loanPayments } from '../db/schema';
...
export class GoogleSheetsService {
  ...
  async createOrUpdateVroomSpreadsheet(userId: string, userName: string): Promise<SpreadsheetInfo> {
    try {
      ...
      const existingSpreadsheet = await this.findVroomSpreadsheet(folderStructure.mainFolder.id, userName);
   ...
   */
  private async findVroomSpreadsheet(mainFolderId: string, userName: string): Promise<any> {
    try {
   ...
   */
  private async updateSpreadsheetWithUserData(spreadsheetId: string, userId: string): Promise<void> {
    try {
      const db = databaseService.getDatabase();

      // Get user data
      const [userVehicles, userExpenses, userInsurance, userLoans, userLoanPayments] = await Promise.all([
        db.select().from(vehicles).where(eq(vehicles.userId, userId)),
        db.select().from(expenses).innerJoin(vehicles, eq(expenses.vehicleId, vehicles.id)).where(eq(vehicles.userId, userId)),
        db.select().from(insurancePolicies).innerJoin(vehicles, eq(insurancePolicies.vehicleId, vehicles.id)).where(eq(vehicles.userId, userId)),
        db.select().from(vehicleLoans).innerJoin(vehicles, eq(vehicleLoans.vehicleId, vehicles.id)).where(eq(vehicles.userId, userId)),
        db.select().from(loanPayments).innerJoin(vehicleLoans, eq(loanPayments.loanId, vehicleLoans.id)).innerJoin(vehicles, eq(vehicleLoans.vehicleId, vehicles.id)).where(eq(vehicles.userId, userId)),
      ]);
   ...
   */
  private async updateDashboardSheet(spreadsheetId: string, vehicles: any[], expenses: any[]): Promise<void> {
   ...
   */
  private async updateVehiclesSheet(spreadsheetId: string, vehicles: any[]): Promise<void> {
    const headers = [
      'Make', 'Model', 'Year', 'License Plate', 'Nickname', 
      'Purchase Date', 'Purchase Price', 'Initial Mileage',
      'Has Loan', 'Loan Company', 'Loan Amount', 'APR', 'Term (Months)', 'Monthly Payment'
    ];
    ...
    const values = [
      ...
      ...vehicles.map(vehicle => [
        ...
        vehicle.loan ? 'Yes' : 'No',
        vehicle.loan?.lender || '',
        vehicle.loan?.originalAmount ? `$${vehicle.loan.originalAmount.toFixed(2)}` : '',
        vehicle.loan?.apr ? `${vehicle.loan.apr}%` : '',
        vehicle.loan?.termMonths || '',
        vehicle.loan?.standardPayment?.amount ? `$${vehicle.loan.standardPayment.amount.toFixed(2)}` : '',
      ])
    ];

    await this.updateSheetData(spreadsheetId, 'Vehicles!A1:N' + (vehicles.length + 1), values);
  }

  /**
   * Update Expenses sheet
   */
  private async updateExpensesSheet(spreadsheetId: string, expenses: any[]): Promise<void> {
    const headers = [
      'Date', 'Vehicle', 'Type', 'Category', 'Description', 'Amount', 
      'Mileage', 'Gallons', 'MPG', 'Cost/Mile'
    ];
    ...
    const values = [
      ...
      ...expenses.map(exp => {
        ...
        const mpg = expense.gallons && expense.mileage ? (expense.mileage / expense.gallons).toFixed(2) : '';
   ...
   */
  private async updateExpenseCategoriesSheet(spreadsheetId: string, expenses: any[]): Promise<void> {
    ...
    await this.updateSheetData(spreadsheetId, 'Expense Categories!A1:C' + (Object.keys(categoryTotals).length + 1), values);
  }

  /**
   * Update Insurance sheet
   */
  private async updateInsuranceSheet(spreadsheetId: string, insurance: any[]): Promise<void> {
    const headers = [
      'Vehicle', 'Company', 'Policy Number', 'Total Cost', 'Term (Months)', 
      'Start Date', 'End Date', 'Monthly Cost', 'Active'
    ];
   ...
   */
  private async updateLoanDetailsSheet(spreadsheetId: string, loans: any[]): Promise<void> {
    const headers = [
      'Vehicle', 'Loan Company', 'Original Amount', 'Current Balance', 'APR', 
      'Term (Months)', 'Monthly Payment', 'Start Date', 'Active'
    ];
    ...
    const values = [
      ...
      ...loans.map(loan => {
        ...
        return [
          ...
          `${loanData.apr}%`,
          loanData.termMonths,
          `$${loanData.standardPayment?.amount?.toFixed(2) || '0.00'}`,
   ...
   */
  private async updateLoanPaymentsSheet(spreadsheetId: string, payments: any[]): Promise<void> {
    const headers = [
      'Vehicle', 'Payment Date', 'Payment #', 'Amount', 'Principal', 
      'Interest', 'Remaining Balance', 'Payment Type'
    ];
   ...
   */
  private async updateMonthlySummarySheet(spreadsheetId: string, expenses: any[]): Promise<void> {
    ...
    await this.updateSheetData(spreadsheetId, 'Monthly Summary!A1:' + String.fromCharCode(65 + headers.length - 1) + (Object.keys(monthlyData).length + 1), values);
  }

  /**
   * Update sheet data
   */
  private async updateSheetData(spreadsheetId: string, range: string, values: any[][]): Promise<void> {
    try {
   ...
   */
  async exportData(spreadsheetId: string, format: 'json' | 'csv' | 'xlsx'): Promise<Buffer | object> {
    try {
      switch (format) {
        case 'json':
