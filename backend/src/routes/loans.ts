...
import type { NewVehicleLoan, NewLoanPayment } from '../db/schema';
...
const createLoanSchema = z.object({
  ...
  termMonths: z.number().int().min(1, 'Term must be at least 1 month').max(600, 'Term cannot exceed 600 months'),
...
const updateLoanSchema = createLoanSchema.partial().omit({ vehicleId: true });
...
loans.post('/vehicles/:vehicleId/loan', 
  ...
  async (c) => {
    try {
      ...
      const newPayment: NewLoanPayment = {
        ...
        remainingBalance: Math.max(0, loan.currentBalance - Math.min(breakdown.principalAmount, paymentData.paymentAmount)),
        paymentNumber,
