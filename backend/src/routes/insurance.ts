...
const createInsurancePolicySchema = z.object({
  company: z.string().min(1, 'Insurance company is required').max(100, 'Company name must be 100 characters or less'),
  ...
  termLengthMonths: z.number().int().min(1, 'Term length must be at least 1 month').max(24, 'Term length cannot exceed 24 months'),
...
insurance.post('/vehicles/:id/policies',
...
insurance.get('/vehicles/:id/policies',
  zValidator('param', vehicleParamsSchema),
  async (c) => {
    try {
      const user = c.get('user');
      ...
      const policiesWithAlerts = policies.map(policy => {
        const daysUntilExpiration = Math.ceil((new Date(policy.endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
        
        return {
          ...policy,
          daysUntilExpiration,
          expirationAlert: daysUntilExpiration <= 30 ? {
...
insurance.get('/:id/monthly-breakdown', zValidator('param', insurancePolicyParamsSchema), async (c) => {
  try {
    const user = c.get('user');
    const { id } = c.req.valid('param');
...
function calculateMonthlyBreakdown(policy: any) {
  ...
  while (currentDate < endDate && monthNumber <= policy.termLengthMonths) {
    ...
    breakdown.push({
      ...
      daysInMonth: Math.ceil((monthEnd.getTime() - monthStart.getTime()) / (1000 * 60 * 60 * 24)) + 1
    });
