...
### deep-review
> ~~**Expense write-path + CSV foreign-import mapping audit (C465) → CSV import CERTIFIED CLEAN; found+fixed #145 (the /split route is the THIRD forgeable source-link path #125/C422 missed).**~~ — *DONE C465 (nothing over budget → highest-leverage deep-review). 2-agent fan-out. (B) CSV foreign-import mapping CERTIFIED CLEAN, verified firsthand: #124 last-separator-wins holds (lone-comma = the gated #24), unit-conversion direction/guards sound, detectSource tie convenience-only (user confirms units pre-commit), #102 collision→null + #137 clearImportedFuelFields + #C385 date echo-check all hold, unmapped category → misc + surfaced in unmappedCategories (no silent drop/wrong bucket). (A) EXPENSE write-path found #145 (see bug queue, fixed same-cycle): the manual POST /expenses/split accepted `sourceType: z.string().optional()` (any string) + ran NO assertFinancingSourceValid while the regular POST/PUT restrict to the 'financing' literal + fully validate it (#62/#125). VERIFIED firsthand the rest of the write-path CLEAN: #76/#130 effective-category fuel-clear holds, #109 both-or-neither holds, #125 regular-path assert holds, #141 groupTotal==Σlegs cent-exact on all 3 methods incl. UPDATE, tenant-scoping solid. be validate:local EXIT 0, 1555 pass (+4).*
...
### guard
> ~~**C463 — pin the mileage-reminder due-gate's EXACT-equality boundary (the load-bearing `<` = `>=`-due semantics, via the REAL trigger endpoint).**~~ — *DONE C463 (guard MOST-starved over budget 7>6; arch also over at 6>5 but guard more-starved → forced). 2-agent fan-out: FE scout returned CERTIFY (FE pure/service/store layer genuinely exhausted — only navigation.ts left, below the load-bearing bar; remaining FE gap is eyes-on components/routes, Playwright-blocked). BE scout found the pick (verified firsthand): processMileageReminder's due-gate (trigger-service.ts:418) `currentOdometer < nextDueOdometer → not yet due` means EXACT-equality must FIRE. Reachable via POST /reminders/trigger + the recheck-on-write hook; getCurrentOdometer returns a raw integer MAX(odometer) so an exact-milestone reading is genuinely reachable. trigger-mileage.test.ts's docstring CLAIMS `>=` but every firing case seeds STRICTLY past (35200/36000/40500) → the `=` edge was undriven (the same `>=`-tolerant-but-never-`=` gap as C450/C430). +1 guard: odometer EXACTLY 35000 on a 35000 milestone → fires 1 notification. PROVEN NON-VACUOUS by a temporary `<`→`<=` flip → exactly this test fails (1 fail/7 pass), reverted. be validate:local EXIT 0, 1551 pass (+1). NOTE: backup pick was monthsBetween (analytics/repository.ts:83) — Property-15 is self-referential (numerator+divisor move in lockstep) so a reimplementation regression stays green; queued for a future guard cycle if BE guard comes due before a fresh deep-review-surfaced invariant.*
...
> ~~**#145 (HIGH, money + data-loss / NORTH_STAR #1 — found+fixed C465 on an expense-write-path deep-review; the #62/#109/#125 within-tenant source-link integrity class on the THIRD path those
> fixes never closed) — the manual SPLIT expense route (POST /expenses/split) accepted an UNVALIDATED arbitrary source link.**~~ — *DONE C465: createSplitExpenseSchema had `sourceType:
> z.string().optional()` (ANY string, validation.ts:115) + the /split handler ran NO assertFinancingSourceValid — while the regular POST/PUT restrict sourceType to z.literal('financing') AND fully
> validate it (#62/C190 + #125/C422). VERIFIED FIRSTHAND: split-validation-schema.test.ts:130 even ASSERTED sourceType:'reminder' parses success:true. Two reachable harms from a hand-crafted POST:
> (1) sourceType:'financing'+an arbitrary sourceId → summed by computeBalance per vehicle (financing/repository.ts:118) → UNDERSTATES the displayed loan balance (NORTH_STAR #1 money figure), the
> exact forge assertFinancingSourceValid exists to block; (2) sourceType:'insurance_term'+a real term id → when that policy is deleted, deleteBySource('insurance_term',…) cascade-DELETES the manual
> split rows (silent data-loss of unrelated expenses). Within-tenant (sourceId is a server cuid2, all writes userId-scoped) — not cross-tenant. FIX (mirror the regular path): tightened the schema's
> sourceType to z.literal('financing').optional() + sourceId z.string().min(1) (closes the non-financing forge incl. the insurance_term cascade vector), + added assertFinancingSourceValid per DISTINCT
> split vehicleId in the POST handler (each sibling lands on its own vehicle → each must point at THAT vehicle's active financing). The split PUT carries no source fields (re-stamps the existing
...
### arch
- ~~**Delete the dead Lucia test-override seam — setTestLucia + the provably-null testLucia state; collapse getLucia to `return lucia` (zero-ref EXPORTED-symbol delete, rule-7 FRESH fan-out, done C464).**~~ — *DONE C464 (arch SOLE over-budget 7>5, forced). rule-7 FRESH fan-out (per C457's "fan out fresh, don't re-scout the certified dead-ends"): FE scout CERTIFIED the pure/service/store layer dry (best candidate getDaysRemaining/calculateDaysUntil = cross-domain cohesion-negative churn, rejected; no dead code). BE scout found the pick (verified firsthand): setTestLucia (auth/lucia.ts:73) is an EXPORTED test-override setter with ZERO callers anywhere incl. __tests__/test-helpers (tests drive the real Lucia via createTestApp's session cookie) — and the ONLY writer of module-state `testLucia` (refs = decl :67, getLucia read :70, this write :74) → testLucia provably ALWAYS null → getLucia() (12 callers) always returns the real `lucia`. A true zero-ref delete (NOT the C438 coverage-dropping anti-trap; NOT a mere unexport). Deleted setTestLucia + the dead `testLucia` let, collapsed getLucia to `return lucia` (kept the 12-caller seam), documented why. rule-2 behavior-preserving; rule-3 green→green, NO test touched. −8 LOC. be validate:local EXIT 0, 1551 pass (UNCHANGED — confirms dead). BE backup for a future arch cycle: calculateMPG is an unexport-only candidate (used once internally), low payoff.*
...
- ~~**CLAUDE.md full refresh (C467) — the overdue ~C464 cadence (last full refresh C454).**~~ — *DONE C467 (infra SOLE over-budget 7>6; the refresh deferred at C464 to keep that cycle to its ONE arch increment). Doc-only, no source. Refreshed the drifted snapshot: COVERAGE line → the C460 full re-measure (be 87.09/86.60 · fe 85.89/87.15/78.35, gap ~1.2pts; was carrying C447's 86.96/86.55); SUITE SIZE → ~1558 BE / ~715 FE (was ~1543/~714) + re-pointed the recent-arc note to C459–C466; CLOSED-BUG ARC → appended #142/#144/#145/#146 + the #143 grammar + C464 Lucia delete, bumped "all landed C155→C466"; PENDING-ANGELO/CLOSED block → reset to the C447→C466 window, recorded the #62/#109/#125 source-link class closed across ALL 3 write paths + the #56/#108/#113 split-sibling sweep, flagged #140 as the lone clean-but-eyes-on open item. Verified no stale C453/1543/C447 markers remain. Next CLAUDE.md full refresh ~C477.*

- ~~**#5 branch-hygiene sweep + coverage re-measure + BRANCH_REVIEW.md refresh (C460)**~~ — *DONE C460 (infra most-starved 6/6; cadence overdue, last actual sweep C447).
  (1) zero stray untracked unit/spec .test.ts; branch 270 commits, 0 behind, clean fast-forwardable. (2) green baseline + RE-MEASURE: be 87.09% line / 86.60% func (1549 pass, +0.13
  line vs C447 — the C453/C458/C459 fix+guards); fe 85.89% line / 87.15% func / 78.35% branch (714 pass, flat vs C447); BE↔FE gap ~1.2pts. (3) BRANCH_REVIEW.md refresh: 257→270
  commits, +§35 (the 13-commit C447–C459 delta — 4 bugs incl. #142 feature-disabling mileage-feed + #141 split-cents, two CLASSES closed [#139 0%-APR, #140 annual-vs-total]; the
  C452/C458 certs; the C457 dedup-exhausted cert). Doc/measurement-only. Next #5 ~C470; CLAUDE.md ~C464.*
