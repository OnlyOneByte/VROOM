...
*(queue empty — repopulate as loop tooling / docs needs surface. #5 branch-hygiene sweep DONE C203 (post-squash branch 13 commits C190–C202; §27; re-measure be 84.06% / fe 77.79% — FE up sharply), next due ~C213; CLAUDE.md refresh DONE C208 (3 drifts: C203 coverage re-measure, suite 1274/557, closed-bugs #64–#67), next due ~C220. NOTE C164/C165 (still standing): feature chronically starved (eyes-on-blocked + money-cents/trips both T0-gated) — escalated to Angelo for a horizon-spec steer + the T0 ratify; trips T1–T5 are loop-buildable once cleared.)*
