...
> clean), ~~`auth.ts` ~56%~~ DONE C217 (+4: requireAuth — the previously-untested per-page guard's sync authed/unauthed branches + BOTH
> loading-poll paths via fake timers; the existing test covered only route-classification + handleRouteProtection);
> `settings.svelte.ts` ~11% is the filed handleError arch pick (deferred). **NEXT FE guard pick (no primed): sync-manager.ts
> ~56% (timer/network-bound — less clean); the FE pure/service modules are now essentially all covered.** The components/routes deficit is
...
*(queue empty — repopulate as loop tooling / docs needs surface. #5 branch-hygiene sweep DONE C213 (post-squash branch 23 commits C190–C212; §28; re-measure be 84.14% / fe 79.22% — FE service layer now fully covered, BE↔FE gap nearly closed), next due ~C223; CLAUDE.md refresh DONE C219 (5 drifts: C213 coverage re-measure, FE-ratchet+next-low-spot, suite 1300/580, closed-bugs #68–#73, #69 escalation), next due ~C232. NOTE C164/C165 (still standing): feature chronically starved (eyes-on-blocked + money-cents/trips both T0-gated) — escalated to Angelo for a horizon-spec steer + the T0 ratify; trips T1–T5 are loop-buildable once cleared.)*
